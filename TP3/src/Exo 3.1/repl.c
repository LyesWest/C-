...
int nombre_commandes = sizeof(commandes) / sizeof(commandes[0]);
    ...
    while (continuer)
    {
...
} else if (strcmp(commandes[i].nom, "quit") == 0) {
                    traiter_quit();
                    continuer = 0;
                } else {
        ...
        if (!commande_reconnue) {
            ...
            printf("Commande non reconnue. Essayez 'echo <text>' pour afficher du texte, 'date' pour la date actuelle, 'h>
        }
...
void afficher_aide()
{
    ...
    printf("  quit    - Quitter l'interpréteur de commandes\n");
